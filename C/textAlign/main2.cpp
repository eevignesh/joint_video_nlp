 ...
 *
 * @output:
 * nromalized Levenshtein distance
 *
 * */
...
/*
 * @brief: fucntion to compute the time synchronized distance between two set of sequential strings
 *
 * @paramters:
...
float getTimeSynchDistance(vector<string> sub, vector<string> script, float (*distFunction)(string, string), bool isVerbose = true)
{
  ...
  // compute the distances (not required)
	...
	for(int n2 = 0; n2 < N2; n2++)
      ...
      dists[n2][n1] = 1 - (*distFunction)(sub[n2], script[n1]);
 ...
 * */
float timeSynchDistance(string sub, string script)
{
  ...
  // now call the time synched. distance function wiht levenshtein measure
  return -getTimeSynchDistance(tokens_sub, tokens_script, normalized_levenshtein_distance, false);
}
...
int main(int argc, char* argv[]){
  ...
  getTimeSynchDistance(sub, script, timeSynchDistance);
  return 0;
